...
for (i in 1:nrow(pairs.to.test)) {
    ...
    snp.1 = pairs.to.test[i,]$snp_1
    snp.2 = pairs.to.test[i,]$snp_2
    ...
    tissue.test = data.frame()
    for(tissue in unique(haplotype.mean$Tissue)) {
      ...
      # Return the results
      tissue.test = rbind(tissue.test, results)
    }
    ...
    # Write to a row
    row = tibble(snp_1 = snp.1, snp_2 = snp.2, background, haplotype, lik.g1, lik.g2)
    background.test.df = rbind(background.test.df, row)
