...
The goal of this notebook is to establish a method to genotype SNPs as either belonging to `Genome 1` or to `Genome 2`. **The output of this notebook is a set of phased variants with genome 1 and genome 2 background assignments renamed so that haplotypes are in sequential order.**
...
if (exists("snakemake")) {
  assigned.output.path = snakemake@params[["assigned"]]
  assigned.output.path = snakemake@params[["backgrounds"]]
} else {
  # G1 and G2 assigned variants
  assigned.output.path = "../../results/variants/assigned_variants.csv"
  # Assignments for each cluster SNP 
  background.output.path = "../../results/bridging/cluster_snp_backgrounds.csv"
...
write_csv(assigned.df, assigned.output.path)

```

```{r Write out the SNP backgrounds, echo=T}

write_csv(background.test.df, background.output.path)
