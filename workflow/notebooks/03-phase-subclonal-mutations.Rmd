...
promising.subclonal.clusters = c(20, 21, 27, 25)
...
```{r Figure 4, fig.align='center', fig.width=14, fig.height=7, echo=FALSE}
...
core.snps = figure_4.df %>% 
  filter(Genotype != 'subclonal' & Driver == "")

subclonal.snps = figure_4.df %>% 
  filter(Genotype == 'subclonal' & Driver == "")

driver.snps = figure_4.df %>% 
  filter(Driver != "")
...
ggsave(paste(figure.dir, "figure-4.png"), width = 14, height = 7, dpi = 300)
...
```{r Figure 5 Without G-FC2, fig.align='center', fig.width=15, fig.height=7, echo=FALSE}

figure.5.df = labeled.df %>% 
  mutate(Genotype = case_when(
...
ggsave(paste(figure.dir, "figure-5-without-GFC2.png"), width = 15, height = 8, dpi = 300)
