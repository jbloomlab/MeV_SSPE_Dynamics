...
The goal of this notebook is to identify other clusters of mutations with same approach used to define mutations in Genome-1 and Genome-2. Hopefully, these can be used to establish some base sub clonal haplotypes. 
...
labeled.data = "../../results/spatial/labeled_variants.csv"
...
# SNPs that are above 20% at some point in some tissue
...
Now, lets look at the remainder of the clusters of SNPs. 

```{r Remaining Clusters, fig.align='center', fig.width=19, fig.height=10}
...
16 is probably the same as 1, just low coverage in the Cerebellum and Cerebellum Nucleus.

10 and 15 are likely fixed, just missing because of low coverage. We'll call these fixed for now. 
...
What clusters are left? There are three with a single mutation. Also, there are lots of mutations in the frontal cortex at high frequency. 

None of these single mutations are all the important. There are some mutations that are probably in the 3' and 5' UTR. The other mutation is probably Synonymous, but it's in P/V/C, so it could interrupt another reading frame. 

```{r Single Mutations, fig.align='center', fig.width=12, fig.height=6}
...
updated.haplotype.df = updated.haplotype.df %>%
  ...
  mutate(Haplotype = case_when(SNP %in% cluster.10.snps ~ "cluster 10",
                               ...
                               SNP %in% cluster.14.snps ~ "cluster 15",
                               TRUE ~ Haplotype)) 
