...
ggsave(paste(figure.dir, "extended-data-figure-9.png", sep=""), width = 19, height = 10)

```

```{r Ext Figure 9, message=F, warning=F, fig.align='center', fig.width=19, fig.height=10, echo=F}
...
expanded.df %>% 
    filter(!Haplotype %in% c("fixed", "both", "subclonal")) %>% 
    mutate(Genotype = case_when(
      Background == "genome-1" ~ "G1",
      Background == "genome-2" ~ "G2"
    )) %>% 
...
```{r Figure 7a, message=F, warnings=F, fig.align='center', fig.width=15, fig.height=10}
...
distribution.df %>% 
  ggplot(aes(x = POS, y = factor(Haplotype, levels = rev(figure.haplotype.order)), col = Haplotype)) +
    ...
    scale_x_continuous(limits=c(-400, 16250), breaks=c(0, 5000, 10000, 15000)) +
    ...
    # 3' Start
    annotate(geom = "text", x = (N$Start - 400), y = .05, label = "3'", size = 8) +
    ...
    # 5' Start
    annotate(geom = "text", x = (L$Stop + 400), y =  .05, label = "5'", size = 8) +
...
ggsave(paste(figure.dir, "figure-7a.png", sep=""), width = 11.25, height = 10, dpi = 300)
