...
ggsave(paste(figure.dir, "extended-data-figure-4.png", sep=""), width = 19, height = 10)
...
g1.snps = labeled.df %>% 
  ...
  mutate(Haplotype = "G-1") %>% 
  distinct() 

# Genome-FC2 SNPS
gFC2.snps = labeled.df %>% 
  filter(Haplotype %in% c("cluster 6", "genome-1")) %>% 
  select(Background, Haplotype, Gene, Effect, POS, REF, ALT) %>% 
  mutate(Gene = if_else(Gene == "P/V/C", "P", Gene)) %>% 
  mutate(Effect = if_else(is.na(Effect), "Synonymous", Effect)) %>% 
  mutate(ADAR = case_when(
...
# Haplotype Colors
haplotype.colors =  c("#000000", "#87CEEB", "purple", "#22488F", "#B02B23", "#113061" , "magenta" , "dark grey",  "dark grey", "dark grey", "dark grey", "dark grey", "dark grey", "dark grey" ,"dark grey", "dark grey", "dark grey", "dark grey", "dark grey")
names(haplotype.colors) = c("SSPE Ancestor", "G-01", "G-FC2", "G-1", "G-2", "G-01 subcluster a", "G-01 subcluster b", "cluster 1", "cluster 1a", "cluster 2", "cluster 3", "cluster 4", "cluster 5", "cluster 6", "cluster 7", "cluster 8", "cluster 9", "cluster 10", "cluster 11")
...
ggsave(paste(figure.dir, "figure-6.png", sep=""), width = 15, height = 10, dpi = 300)
