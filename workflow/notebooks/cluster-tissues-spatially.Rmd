...
if (exists("snakemake")) {
  ...
  # Brain coordinates for plotting spatial heatmap
  brain.coords = snakemake@params[["brain_coords"]]
  ...
  # Brain coordinates for plotting spatial heatmap
  brain.coords = "../../config/brain-coordinates.csv"
...
haplotype.colors =  c("#000000", "#E1EAF6",  "#A6C9DF", "#7AADD2","#5691C1", "#3871B0", "#67649F", "#8484B1", "#FFFCBA", "#F9D984", "#F4B460", "#EE9B4F", "#DE4B32", "#22488F", "#B02B23")
names(haplotype.colors) = c("G-FC2", "cluster 1", "cluster 1a", "cluster 2", "cluster 3", "cluster 4", "cluster 5", "cluster 6", "cluster 7", "cluster 8", "cluster 9", "cluster 10", "cluster 11", "G-1 unknown", "G-2 unknown")
...
# Get the background of each haplotype
backgrounds = haplotype.mean %>%
  ...
  mutate(Background = case_when(
    Background == "genome-1" ~ "G-01",
    Background == "genome-2" ~  "G-2",
    TRUE ~ Background))

# Mean G1/G2 frequency in each tissue sample
g1.g2.frequency = haplotype.mean %>% 
  select(!Background) %>% 
  filter(!Haplotype %in% c("subclonal", "both")) %>% 
  pivot_wider(names_from = Haplotype, values_from = AF) %>%
  mutate(`cluster 1` = `cluster 1` - `cluster 1a`) %>% 
  mutate(`cluster 1` = if_else(`cluster 1` < 0, 0, `cluster 1`)) %>% 
  pivot_longer(cols = -c(Tissue), names_to = "Haplotype", values_to = "AF") %>% 
  filter(Haplotype %in% c("G-2", "G-01")) %>%
  ungroup() %>%
  group_by(Tissue) %>%
  mutate(Backgound_AF = AF / sum(AF)) %>% 
  select(!AF) %>% 
  rename(Background = Haplotype)

# Add in a row for unaccounted genome-1 % in each tissue sample
unknown.frequency = cluster.frequencies %>% 
  left_join(., backgrounds, by="Haplotype") %>% 
  group_by(Tissue, Background) %>% 
  summarize(AF = sum(AF)) %>% 
  left_join(., g1.g2.frequency, by=c("Background", "Tissue")) %>% 
  mutate(AF = Backgound_AF - AF) %>% 
  mutate(Haplotype = case_when(
    Background == "G-01" ~ "G-1 unknown",
    Background == "G-2" ~ "G-2 unknown",
  )) %>% 
  ungroup() %>% 
  select(!c("Background", "Backgound_AF"))
...
brain.coordinates.df = read_csv(brain.coords, show_col_types = F)
cluster.frequencies = cluster.frequencies %>% 
...
ggplot() +
    geom_scatterpie(aes(x=x, y=y, r=Size*2.7, group=Tissue), 
...
ggsave(paste(figure.dir, "figure-7b-less-size-range.png"), width = 8, height = 4, dpi = 300)
