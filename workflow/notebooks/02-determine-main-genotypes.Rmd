...
if (exists("snakemake")) {
  ...
  # Path to the SSPE consensus mutations
...
# The consensus is made with all samples. 
# This can lead to double counting if a 
# variant in the SSPE 1 sample wasn't >98% 
# Remove anything that is being double counted
ref.snps = selected.variants.df %>% 
  filter(Tissue == "SSPE 1") %>% 
  select(POS, REF, ALT, AF) %>% 
  filter(POS %in% ancestral.df$POS) %>% 
  left_join(., select(ancestral.df, POS, REF, ALT),
            by = "POS",
            suffix = c(".sspe", ".ancestor")) %>% 
  mutate(SNP = paste0(REF.sspe, POS, ALT.sspe)) %>% 
  pull(SNP)
...
sspe.1.df = selected.variants.df %>% 
  ...
  filter(Tissue == "SSPE 1") %>% 
  # Filter out SNPs also in the brain reference
  filter(!SNP %in% ref.snps) %>% 
...
```{r Plot Pilot Data, warning=F, echo = F, fig.align = 'center', fig.width = 10, fig.height = 5}

# Plot the pilot data with sample SSPE 1
...
ggsave(paste(figure.dir, "figure-2.png", sep = ""), width = 11, height = 5, dpi = 300)
...
How well did the frequency bins stack up with the real assignments? 

```{r Format Annotated SSPE Data, echo = F, warning = F}

# Read in the consensus SNPs
ancestral.df = read_csv(ancestral.data, show_col_types = FALSE) %>% 
  mutate(SNP = paste0(REF, POS, ALT)) %>% 
  select(POS, REF, ALT, SNP) %>% 
  mutate(AF = 1.0, Haplotype = "Ancestral") 

# Format the data for plotting
sspe.2.df = labeled.df %>% 
  # Select the pilot specimen
  filter(Tissue == "SSPE 2") %>% 
  # Select the important columns 
  select(colnames(ancestral.df)) %>% 
  # Add in the ancestral SNPs 
  bind_rows(., ancestral.df) %>% 
  ...
  mutate(Genotype = case_when(
    AF >= 0.9 ~ "CBA",
    Haplotype == "genome-1" ~ "CG1", 
    Haplotype == "genome-1-1" ~ "CG1", 
    Haplotype == "genome-2" ~ "CG2", 
    TRUE ~ "subclonal"
  )) %>% 
  # Point size should be small for mutation we don't know about
  mutate(Size = if_else(Genotype == "subclonal", "small", "normal")) %>% 
...
# Binned SNPS (i.e. CBA, CG1, and CG2)
binnned.snps = sspe.2.df %>% 
  filter(Genotype %in% c("CBA", "CG1", "CG2") & Driver == "")
# Un-binned SNPS (i.e. subclonal)
subclonal.snps = sspe.2.df %>% 
  filter(Genotype == 'subclonal' & Driver == "")
# Putative driver SNPs
driver.snps = sspe.2.df %>% 
  filter(Driver != "")

```

```{r Plot Annotated SSPE Data, warning=F, echo = F, fig.align = 'center', fig.width = 10, fig.height = 5}
