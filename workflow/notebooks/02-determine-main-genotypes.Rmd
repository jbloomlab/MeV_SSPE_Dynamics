...
The goal of this notebook is to formalize an approach to phase these sets of mutations by clustering variants that have strongly correlated frequencies across multiple tissue samples using k-medoids clustering on a correlation matrix of allele frequeny. 
...
We can see these mutations clearly in the SSPE 1 pilot sample if we draw 'gates' of frequency around mutations on the plot. Below, I'm drawing 'gates' around 60-75% and 30-45% of the genome respectively.
...
ggplot(data = binnned.snps, aes(x = POS, y = AF, color = Genotype, shape = Shape)) +idh
  geom_point(size = 2) + 
