...
updated.df %>% 
  ...
  ggplot(aes(x = POS, y = factor(Haplotype, levels = rev(haplotype.order)), col = Background)) +
    geom_point(aes(shape = ADAR), size = 6) +
    xlab("Position") +
  
  
    scale_x_continuous(limits=c(0,16000)) +
    scale_color_manual(values=c("#307EC2", "#B63F3E", "black")) +
    scale_shape_manual(values = c(108, 4)) +
...
ggsave("../../results/figures/mutations-per-haplotype.png", width =  15, height = 10, dpi = 300)
