...
# Get the mean frequency of the major genomes for plotting
...
# Define more clusters 

```{r Look for Haplotypes, fig.align='center', fig.width=10, fig.height=8}
...
haplotype_df %>% 
  ...
  ggplot(aes(x = POS, y = AF, col = Haplotype, shape = Background)) + 
    geom_point(size = 2.5) + 
    facet_grid(rows = vars(HAPID), cols = vars(reorder(Interval, Start))) + 
    theme_bw(18)
