...
haplotype.clusters = haplotype.mean %>% 
  filter(Background %in% c("genome-1", "genome-2") & (!Haplotype %in% c("genome 1", "genome 01", "genome 2"))) %>% 
  pull(Haplotype) %>% 
  unique()
...
haplotype.mean %>% 
  ...
  mutate(`cluster 1` = `cluster 1` - `cluster 5`) %>% 
...
ggsave(paste(figure.dir, "figure-5bc.png", sep=""), width = 8, height = 10)
