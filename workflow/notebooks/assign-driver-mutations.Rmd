---
title: "6. Assign Driver Mutations"
author:
    - "Will Hannon"
...
We've already know about two of these SNPs - `C7036T` and `T7293C` - that we believe are on both genome-1 and genome-2. Of the remaining four *new* SNPs without phylogenetic context, only `G6775A` and `G7033T` are high-enough frequency to reasonable assign to a background. 

## G7033T

This is an interesting SNP because, like `C7036T`, it's a stop codon in the C-terminal domain of the Fusion protein. 
...
There are three tissues that have this mutation at an appreciable frequency. 
...
Are there any bridging reads in these tissues? Below, I filtered the bridging read counts for only reads that: 
1. Overlap position 7033 
...
What's really interesting here is that there does appear to be strong evidence that this stop codon mutation (`F-526*`) is **not** linked to reads with the other possible stop codon driver at position `7036` (`F-527*`). This makes sense biologically if this mutation is selected for adaptive reasons. 
...
Can we say anything about this SNP based on driving reads? 
...
```
Other than being excluded from cluster 13, it's not super clear how to fit this mutation onto the phylogeny. 
